-- ============================================================
-- LIMPA TUDO + RECRIA com versões corretas (013 + 014)
-- ============================================================

-- 1. DROP todas as funções para eliminar sobrecarga
...
-- 2. Tabela de convites (se não existir)
...
BEGIN
  ...
  FROM (
    SELECT json_build_object('id', g.id, 'nome', g.nome, 'slug', g.slug, 'valor_aposta', g.valor_aposta, 'admin_nome', g.admin_nome, 'role', m.role, 'criado_em', g.criado_em) AS q
...
BEGIN
  ...
  FROM (
    ...
    FROM membros_grupo m WHERE m.grupo_id = p_grupo_id ORDER BY m.role, m.usuario_id
  ) sub;
  RETURN v;
END;
$$;
...
BEGIN
  IF NOT usuario_pertence_ao_grupo(p_usuario, p_grupo_id) THEN
    RAISE EXCEPTION 'Acesso negado: voce nao pertence a este grupo';
  END IF;
  SELECT COALESCE(json_agg(sub.q), '[]'::JSON) INTO v
  FROM (
    SELECT json_build_object('usuario_id', m.usuario_id, 'role', m.role, 'pago', m.pago) AS q
...
BEGIN
  ...
  SELECT COALESCE(json_agg(sub.q), '[]'::JSON) INTO v
  FROM (
    SELECT json_build_object('id', g.id, 'nome', g.nome, 'slug', g.slug, 'admin_nome', g.admin_nome, 'valor_aposta', g.valor_aposta, 'criado_em', g.criado_em) AS q
...
BEGIN
  ...
  INSERT INTO membros_grupo (grupo_id, usuario_id, role) VALUES (v_grupo_id, p_usuario, 'participante') ON CONFLICT DO NOTHING;
...
RETURNS JSON LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
  IF NOT usuario_eh_admin_grupo(p_admin, p_grupo_id) THEN RAISE EXCEPTION 'Apenas admin do grupo pode remover'; END IF;
  DELETE FROM membros_grupo WHERE grupo_id = p_grupo_id AND usuario_id = p_usuario;
  RETURN json_build_object('ok', true);
END;
$$;

CREATE OR REPLACE FUNCTION atualizar_grupo(p_grupo_id UUID, p_admin TEXT, p_valor NUMERIC, p_pontos_cheio INT DEFAULT 5, p_pontos_vencedor INT DEFAULT 3, p_admin_password TEXT DEFAULT NULL)
RETURNS JSON LANGUAGE plpgsql SECURITY DEFINER AS $$
...
BEGIN
  ...
  ELSIF p_admin_password IS NOT NULL THEN
    SELECT admin_password INTO v_admin_cfg FROM config WHERE id = 1;
    IF v_admin_cfg IS NULL OR v_admin_cfg = '' OR p_admin_password IS DISTINCT FROM v_admin_cfg THEN
      RAISE EXCEPTION 'Acesso negado: senha invalida';
    END IF;
  ELSE
    RAISE EXCEPTION 'Acesso negado';
  END IF;
  UPDATE grupos SET valor_aposta = p_valor, pontos_acerto_cheio = p_pontos_cheio, pontos_acerto_vencedor = p_pontos_vencedor WHERE id = p_grupo_id;
  RETURN json_build_object('ok', true);
END;
$$;
...
CREATE OR REPLACE FUNCTION gerar_convite_grupo(p_usuario TEXT, p_grupo_id UUID, p_validade_dias INT DEFAULT 7, p_max_usos INT DEFAULT 0)
RETURNS JSON LANGUAGE plpgsql SECURITY DEFINER AS $$
...
BEGIN
  ...
  RETURN json_build_object('id', v_convite.id, 'token', v_convite.token, 'grupo_id', v_convite.grupo_id, 'expira_em', v_convite.expira_em, 'max_usos', v_convite.max_usos, 'ativo', v_convite.ativo);
END;
$$;
...
DECLARE v JSON;
BEGIN
  IF NOT usuario_eh_admin_grupo(p_usuario, p_grupo_id) THEN RAISE EXCEPTION 'Apenas admin pode ver convites'; END IF;
  SELECT COALESCE(json_agg(sub.q), '[]'::JSON) INTO v
  FROM (
    SELECT json_build_object('id', c.id, 'token', c.token, 'expira_em', c.expira_em, 'max_usos', c.max_usos, 'usos', c.usos, 'ativo', c.ativo, 'criado_por', c.criado_por, 'created_at', c.created_at) AS q
    FROM convites_grupo c WHERE c.grupo_id = p_grupo_id ORDER BY c.created_at DESC
  ) sub;
  RETURN v;
END;
$$;
...
BEGIN
  ...
  IF NOT usuario_eh_admin_grupo(p_usuario, v_grupo_id) THEN RAISE EXCEPTION 'Apenas admin pode revogar'; END IF;
...
RETURNS JSON LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE v_convite convites_grupo; v_grupo_nome TEXT;
BEGIN
  IF NOT usuario_existe(p_usuario) THEN RAISE EXCEPTION 'Usuario nao encontrado'; END IF;
  SELECT * INTO v_convite FROM convites_grupo WHERE token = p_token;
  IF v_convite.id IS NULL THEN RAISE EXCEPTION 'Convite invalido'; END IF;
  IF NOT v_convite.ativo THEN RAISE EXCEPTION 'Convite revogado'; END IF;
  IF v_convite.expira_em < NOW() THEN RAISE EXCEPTION 'Convite expirado'; END IF;
  IF v_convite.max_usos > 0 AND v_convite.usos >= v_convite.max_usos THEN RAISE EXCEPTION 'Convite esgotado'; END IF;
  ...
  END IF;
  INSERT INTO membros_grupo (grupo_id, usuario_id, role) VALUES (v_convite.grupo_id, p_usuario, 'participante');
...
DECLARE v JSON;
BEGIN
  IF NOT usuario_pertence_ao_grupo(p_usuario, p_grupo_id) AND NOT EXISTS (SELECT 1 FROM jogadores WHERE nome = p_usuario AND is_ia = TRUE) THEN
    RAISE EXCEPTION 'Acesso negado';
  END IF;
  SELECT COALESCE(json_agg(sub.q), '[]'::JSON) INTO v
  FROM (
...
BEGIN
  ...
  IF NOT usuario_eh_admin_grupo(p_usuario, v_grupo_id) THEN RAISE EXCEPTION 'Apenas admin pode validar'; END IF;
