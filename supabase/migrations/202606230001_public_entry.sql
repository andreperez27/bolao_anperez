-- =============================================================
-- Migration: Public Entry Flow (no token required)
-- =============================================================

-- 1. buscar_grupo_publico — retorna dados básicos do grupo (sem auth)
...
-- 2. solicitar_entrada_grupo — cadastro + solicitação em um passo (sem token)
...
BEGIN
  ...
  IF FOUND THEN
    ...
    END IF;
    -- Se não tem sessao_token, gerar um novo
  ...
  -- 4. Verificar join request existente
  ...
  IF FOUND THEN
    ...
    ELSIF v_existing_status IN ('approved', 'blocked', 'cancelled') THEN
      -- Casos raros: se approved mas sem group_members, criar
...
-- 3. verificar_status_participante — consulta status sem criar nada
